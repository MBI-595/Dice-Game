...
def generateImage(ifn, txtfn, imageSize, diceSize):
    ...
    with open(txtfn, 'w') as f:
        f.write(
            f'{imgInd1-1}\t{(x1+ofs)/imageSize}\t{(y1+ofs)/imageSize}\t{(diceSize)/imageSize}\t{(diceSize)/imageSize}\n')
        f.write(
            f'{imgInd2-1}\t{(x2+ofs)/imageSize}\t{(y2+ofs)/imageSize}\t{(diceSize)/imageSize}\t{(diceSize)/imageSize}')


for i in range(1, 301):
    generateImage('Data/train/images/'+str(i)+'.png',
                  'Data/train/labels/'+str(i)+'.txt', 200, randint(20, 30))

for i in range(1, 201):
    generateImage('Data/valid/images/'+str(i)+'.png',
                  'Data/valid/labels/'+str(i)+'.txt', 200, randint(20, 30))
